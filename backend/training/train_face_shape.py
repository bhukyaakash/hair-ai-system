"""Train Face Shape Classifier Model - FIXED VERSION"""
...
# Face shapes
FACE_SHAPES = ["round", "oval", "square", "heart", "oblong", "diamond"]
...
def build_face_shape_model():
    """Build EfficientNetB0-based face shape classifier"""
    logger.info("🏗️  Building EfficientNetB0-based model...")
...
def train_face_shape_model():
    ...
    logger.info("="*70)
    logger.info("🚀 Starting Face Shape Classifier Training...")
    logger.info("="*70)
    ...
    try:
        ...
        try:
            # Try to load from real data directory
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(
                os.path.join(MODEL_SAVE_PATH, 'face_shape_best.h5'),
        ...
        # Save H5 format
        model.save(os.path.join(MODEL_SAVE_PATH, 'face_shape_model.h5'))
        logger.info(f"✓ H5 model saved: {os.path.join(MODEL_SAVE_PATH, 'face_shape_model.h5')}")
        ...
        with open(os.path.join(MODEL_SAVE_PATH, 'face_shape_model.tflite'), 'wb') as f:
            f.write(tflite_model)
        logger.info(f"✓ TFLite model saved: {os.path.join(MODEL_SAVE_PATH, 'face_shape_model.tflite')}")
        ...
        with open(os.path.join(MODEL_SAVE_PATH, 'face_shape_metrics.json'), 'w') as f:
            json.dump(metrics, f, indent=2)
        logger.info(f"✓ Metrics saved: {os.path.join(MODEL_SAVE_PATH, 'face_shape_metrics.json')}")
        
        logger.info("\n✅ Face Shape Training COMPLETED SUCCESSFULLY!")
        logger.info("="*70)
...
if __name__ == "__main__":
    success = train_face_shape_model()
