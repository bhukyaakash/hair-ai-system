"""Train Hair Health Analyzer Model - FIXED VERSION"""
...
HEALTH_CATEGORIES = ["healthy", "dry", "oily", "damaged"]
...
def build_hair_health_model():
    """Build DenseNet121-based hair health analyzer"""
    logger.info("🏗️  Building DenseNet121-based model...")
...
def train_hair_health_model():
    """Train hair health analyzer"""
    logger.info("="*70)
    logger.info("🚀 Starting Hair Health Analyzer Training...")
    logger.info("="*70)
    ...
    try:
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(
                os.path.join(MODEL_SAVE_PATH, 'hair_health_best.h5'),
        ...
        model.save(os.path.join(MODEL_SAVE_PATH, 'hair_health_model.h5'))
        ...
        with open(os.path.join(MODEL_SAVE_PATH, 'hair_health_model.tflite'), 'wb') as f:
            f.write(tflite_model)
        logger.info(f"✓ TFLite model saved")
        
        metrics = {
            "model": "Hair Health Analyzer",
        ...
        with open(os.path.join(MODEL_SAVE_PATH, 'hair_health_metrics.json'), 'w') as f:
            json.dump(metrics, f, indent=2)
        logger.info(f"✓ Metrics saved")
        
        logger.info("\n✅ Hair Health Training COMPLETED SUCCESSFULLY!")
        logger.info("="*70)
...
if __name__ == "__main__":
    success = train_hair_health_model()
